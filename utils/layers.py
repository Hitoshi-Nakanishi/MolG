...
def graph_convolution_layer(inputs, units, training, activation=None, dropout_rate=0.):
    ...
    output = tf.stack([tf.layers.dense(annotations, units) for _ in range(adj.shape[1])], 1)
    output = tf.matmul(adj, output)
    output = tf.reduce_sum(output, 1) + tf.layers.dense(annotations, units)
...
def graph_aggregation_layer(inputs, units, training, activation=None, dropout_rate=0.):
    i = tf.layers.dense(inputs, units, activation=tf.nn.sigmoid)
    j = tf.layers.dense(inputs, units, activation=activation)
...
def multi_dense_layers(inputs, units, training, activation=None, dropout_rate=0.):
    ...
    for u in units:
        hidden_tensor = tf.layers.dense(hidden_tensor, u, activation=activation)
