...
class GraphGANModel(object):
    ...
    def __init__(self, vertexes, edges, nodes, embedding_dim, decoder_units, discriminator_units,
        ...
        self.edges_labels = tf.placeholder(tf.int64, shape=(None, vertexes, vertexes))
        self.nodes_labels = tf.placeholder(tf.int64, shape=(None, vertexes))
        self.embeddings = tf.placeholder(tf.float32, shape=(None, embedding_dim))
        self.rewardR = tf.placeholder(tf.float32, shape=(None, 1))
        self.rewardF = tf.placeholder(tf.float32, shape=(None, 1))
        ...
        with tf.name_scope('outputs'):
            (self.edges_softmax, self.nodes_softmax), (self.edges_argmax, self.nodes_argmax), \
            ...
            self.edges_hat = tf.case({self.soft_gumbel_softmax: lambda: self.edges_gumbel_softmax,
                                     ...
                                     default=lambda: self.edges_softmax, exclusive=True)
            self.nodes_hat = tf.case({self.soft_gumbel_softmax: lambda: self.nodes_gumbel_softmax,
                                     ...
                                     default=lambda: self.nodes_softmax, exclusive=True)
